...
export class Incept {
  ...
  public async getLiquidityPosition(
    userWalletAddress: PublicKey,
    liquidityIndex: number
  ) {
    return (await this.getLiquidityPositions(userWalletAddress))
      .liquidityPositions[liquidityIndex];
  }
  ...
  public async getMintPosition(
    userWalletAddress: PublicKey,
    mintIndex: number
  ) {
    return (await this.getMintPositions(userWalletAddress)).mintPositions[
      mintIndex
    ];
  }
  ...
  public async getCometPosition(
    userWalletAddress: PublicKey,
    cometIndex: number
  ) {
    return (await this.getCometPositions(userWalletAddress)).cometPositions[
      cometIndex
    ];
  }
  ...
  public async getUserAccount(userWalletAddress: PublicKey) {
    return (await this.program.account.manager.fetch(
      (
        await this.getUserAddress(userWalletAddress)
      )[0]
    )) as User;
  }
